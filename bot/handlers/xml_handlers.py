...
async def process_metro_quantity(message: Message, state: FSMContext):
    ...
    try:
        ...
        if not city_name or not all_stations:
            await message.answer(
                "❌ Ошибка: данные о метро не найдены.\n"
                "Пожалуйста, начните выбор метро заново."
            )
        ...
        # Переходим к следующему шагу - дате старта
...
def create_ad_element(product, city: str = "", ad_number: int = 1, metro_station: str = None) -> ET.Element:
    """Создание элемента объявления для Avito"""
    ad = ET.Element("Ad")

    # Базовые поля
    ET.SubElement(ad,
                  "Id").text = f"{getattr(product, 'product_id', 'unknown')}_{ad_number}" if ad_number > 1 else getattr(
        product, 'product_id', 'unknown')
    ET.SubElement(ad, "Title").text = getattr(product, 'title', 'Без названия')

    # Описание с возможным добавлением информации о метро
    description = getattr(product, 'description', '')
    if metro_station:
        description += f"\n\n🚇 Рядом со станцией метро '{metro_station}'"
    ...
    # Адрес - генерируем в зависимости от метода размещения
    address = generate_address(city, ad_number, metro_station)
    ET.SubElement(ad, "Address").text = address

    # Контактный телефон
    contact_phone = getattr(product, 'contact_phone', '')
    if contact_phone:
        ET.SubElement(ad, "ContactPhone").text = contact_phone

    # Параметры для сумок
    bag_type = getattr(product, 'bag_type', '')
    bag_gender = getattr(product, 'bag_gender', '')
    if bag_type:
        param_bag_type = ET.SubElement(ad, "Param")
        ET.SubElement(param_bag_type, "Name").text = "Вид одежды, обуви, аксессуаров"

        # Преобразуем код в читаемое значение для Avito
        bag_type_mapping = {
        ...
        ET.SubElement(param_bag_type, "Value").text = bag_type_mapping.get(bag_type, bag_type)

    if bag_gender:
        param_bag_gender = ET.SubElement(ad, "Param")
        ET.SubElement(param_bag_gender, "Name").text = "Для кого"
        ...
        ET.SubElement(param_bag_gender, "Value").text = gender_mapping.get(bag_gender, bag_gender)
    ...
    # Тип объявления
    sale_type = getattr(product, 'sale_type', '')
    if sale_type:
        sale_type_names = {
            "manufacturer": "Товар от производителя",
            "resale": "Товар приобретен на продажу",
    ...
    # Доставка
    avito_delivery = getattr(product, 'avito_delivery', False)
    delivery_services = getattr(product, 'delivery_services', [])

    if avito_delivery and delivery_services:
        ...
        # Скидка на доставку
        delivery_discount = getattr(product, 'delivery_discount', 'none')
        if delivery_discount != "none":
            discount_elem = ET.SubElement(delivery, "Discount")
            discount_elem.text = delivery_discount

    # Изображения
    all_images = getattr(product, 'all_images', getattr(product, 'images', []))
    if all_images:
        images_element = ET.SubElement(ad, "Images")
        for image_url in all_images[:10]:  # Авито принимает до 10 изображений
            image_elem = ET.SubElement(images_element, "Image")
            image_elem.set("url", image_url)

    # Мультиобъявление
    multioffer = getattr(product, 'multioffer', False)
    if multioffer:
    ...
    start_datetime = getattr(product, 'start_datetime', None)
    if start_datetime:
        ET.SubElement(ad, "DateBegin").text = start_datetime.strftime('%Y-%m-%d')

    # Дополнительные параметры для лучшего отображения на Avito
    add_avito_specific_params(ad, product, metro_station)
...
def generate_address(city: str, ad_number: int = 1, metro_station: str = None) -> str:
    ...
    if metro_station:
        ...
        # Адрес без привязки к городу
        return "Россия"


def add_avito_specific_params(ad: ET.Element, product, metro_station: str = None):
    """Добавление специфических параметров для Avito"""

    # Параметр для типа товара
    param_type = ET.SubElement(ad, "Param")
    ET.SubElement(param_type, "Name").text = "Тип товара"
    ET.SubElement(param_type, "Value").text = "Товар"

    # Параметр для способа оплаты
    param_payment = ET.SubElement(ad, "Param")
    ET.SubElement(param_payment, "Name").text = "Способ оплаты"
    ET.SubElement(param_payment, "Value").text = "Наличные, Банковская карта"

    # Если есть метро, добавляем параметр
    if metro_station:
        param_metro = ET.SubElement(ad, "Param")
        ET.SubElement(param_metro, "Name").text = "Метро"
        ET.SubElement(param_metro, "Value").text = metro_station

    # Параметр для состояния упаковки
    condition = getattr(product, 'condition', '')
    if condition:
        param_package = ET.SubElement(ad, "Param")
        ET.SubElement(param_package, "Name").text = "Состояние упаковки"
        if condition == "new_with_tag":
            ET.SubElement(param_package, "Value").text = "Оригинальная упаковка"
        else:
            ET.SubElement(param_package, "Value").text = "Без упаковки"
...
def get_product_price(product) -> int:
    ...
    if price_type == 'fixed':
        ...
        if price_min and price_max and price_min > 0 and price_max > price_min:
            import random
            return random.randint(price_min, price_max)
        return price_min if price_min and price_min > 0 else 0
    else:
        return 0  # Без цены
...
async def process_placement_type(callback: CallbackQuery, state: FSMContext):
    ...
    if placement_type == "cities":
        ...
        # Размещение по метро - сразу переходим к выбору города
